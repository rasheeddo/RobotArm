...
while StartJog:
    ...
    while X_Btn == True:
        ...
        if OnceTrig == True:
            ...
            PreReadXYZ = RobotArmFWD(PreDeg1,PreDeg2,PreDeg3,PreDeg4,PreDeg5,PreDeg6)
            Yconst = PreReadXYZ[1]
            Zconst = PreReadXYZ[2]
            Xrot_const = 0
            Yrot_const = 0
        ...
        if ((abs(Ax0) > 0.0001) and (ExitLoop == False)):
            ...
            Deg5 = ReadDeg[4]
            Deg6 = ReadDeg[5]
            ReadXYZ = RobotArmFWD(Deg1,Deg2,Deg3,Deg4,Deg5,Deg6)
            ReadX = ReadXYZ[0]
            ...
            Yrot_com = Yrot_const
            Zrot_com = Zrot_const
            XYZ = WorkspaceHorizontalLimitation(Xcom,Ycom,Zcom)
            print("Xcom: %f" %XYZ[0])
            print("Ycom: %f" %XYZ[1])
            print("Zcom: %f" %XYZ[2])
            print("------------------------------")
            if ((XYZ[0] is not None) and (XYZ[1] is not None) and (XYZ[2] is not None)):
                DriveAng = RobotArmINV(XYZ[0],XYZ[1],XYZ[2],Xrot_com,Yrot_com,Zrot_com)
                RunServo(DriveAng[0],DriveAng[1],DriveAng[2],DriveAng[3],DriveAng[4],DriveAng[5])
            else:
    ...
    while XRot_Btn == True:
        ...
        Ycom = Yconst
        Zcom = Zconst
        Xrot_com = map(Ax0,-1,1,-30,30)
        Yrot_com = Yrot_const
        Zrot_com = Zrot_const
        XYZ = WorkspaceLimitation(Xcom,Ycom,Zcom)
        print("Xcom: %f" %Xcom)
        print("Ycom: %f" %Ycom)
        print("Zcom: %f" %Zcom)
        print("------------------------------")
